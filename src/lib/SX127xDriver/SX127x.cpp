...
RFAMP_hal RFAMP;
bool pa20dBmPending = false; // 20dbm check variable is created
...
void SX127xDriver::ConfigLoraDefaults()
{
    ...
    hal.writeRegisterBits(SX127X_REG_OCP, SX127X_OCP_ON | SX127X_OCP_200MA, SX127X_OCP_MASK, SX12XX_Radio_All); // 200ma max current
...
{
    bool want20dBm = false;

    // we will use 128 as special value to indicate the desire to use 20dBm mode, which allows for higher power output at the cost of higher current consumption.
    // This is only relevant for the PA_BOOST pin, which is used when OPT_USE_SX1276_RFO_HF is false.
    // When OPT_USE_SX1276_RFO_HF is true, the maximum power is limited to 17dBm, so this value will be treated as 127 instead, which is the maximum valid power value for that configuration.
    if (Power == 128)
    {
        Power = 127;
        want20dBm = true;
...
void ICACHE_RAM_ATTR SX127xDriver::CommitOutputPower()
{
    ...
    if (pa20dBmPending)
        hal.writeRegister(SX1278_REG_PA_DAC, 0x87, SX12XX_Radio_All);
    else
        hal.writeRegister(SX1278_REG_PA_DAC, 0x84, SX12XX_Radio_All);
